    ...
    mFilter(false)
{
    ...
    connect(ui->cbFadeInactive, &QCheckBox::checkStateChanged, this, [this](Qt::CheckState val){mModel.setFadeInactive(val == Qt::Checked);sniffDel->setFadeInactive(val == Qt::Checked);});
    connect(ui->cbMuteNotched, &QCheckBox::checkStateChanged, this, [this](Qt::CheckState val){mModel.setMuteNotched(val == Qt::Checked);});
    connect(ui->cbNoExpire, &QCheckBox::checkStateChanged, this, [this](Qt::CheckState val){mModel.setNeverExpire(val == Qt::Checked);});
    connect(ui->cbViewBits, &QCheckBox::checkStateChanged, this,
            [this](Qt::CheckState val)
            {
                if (val == Qt::Checked) ui->treeView->setItemDelegate(sniffDel);
                else
                {

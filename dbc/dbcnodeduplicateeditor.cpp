    ...
    ui(new Ui::DBCNodeDuplicateEditor)
{
    ...
    connect(ui->btnDuplicate, &QPushButton::pressed,
        [=]()
        {
            ...
            {
                QMessageBox::question(this, "Invalid Address", "The new address is outside of the valid range.", QMessageBox::StandardButtons(QMessageBox::Ok));
                return;
            }

            if(newBase == lowestMsgId)
            {
                QMessageBox::question(this, "Invalid Address", "The new address is the same as the original.", QMessageBox::StandardButtons(QMessageBox::Ok));
                return;
            }
            ...
            {
                QMessageBox::question(this, "No Messages", "The node has no messages to duplicate.", QMessageBox::StandardButtons(QMessageBox::Ok));
                return;
            }

            if(ui->lineNodeName->text().isEmpty())
            {
                QMessageBox::question(this, "No Name", "The new node needs a name before it can be created.", QMessageBox::StandardButtons(QMessageBox::Ok));
                return;
            }
            ...
            if(nodePtr == nullptr)
            {
                QMessageBox::question(this, "Node Invalid", "There was an problem identifying the selected node.", QMessageBox::StandardButtons(QMessageBox::Ok));
                return;
            }
            ...
            for (int i = 0; i < messagesForNode.count(); i++)
            {
                ...
                {
                    QMessageBox::question(this, "Invalid Address Range", "The new starting address would cause a message to be outside of the valid address range.", QMessageBox::StandardButtons(QMessageBox::Ok));
                    return;
                }
            }
